...
esimd_imul(simd<T0, SZ> &rmd, simd<T1, SZ> src0, U src1) {
  ...
  Product *= src1;
  rmd = Product.bit_cast<T0>().select<1, 1>[0];
  return Product.bit_cast<T0>().select<1, 1>[1];
}
...
esimd_imul(simd<T0, SZ> &rmd, simd<T1, SZ> src0, U src1) {
  ...
  ComputationTy Product = convert<long long>(src0);
  Product *= src1;
  rmd = Product.bit_cast<T0>().select<SZ, 2>(0);
  return Product.bit_cast<T0>().select<SZ, 2>(1);
}
#endif
...
esimd_pln(simd<float, 4> src0, simd<float, SZ> src1, simd<float, SZ> src2,
  ...
  auto Src12 = Src12v.template bit_cast<float, (SZ >> 3), 16>();

  Src12.select<(SZ >> 3), 1, 8, 1>(0, 0) =
      src1.template bit_cast<float, (SZ >> 3), 8>();
  Src12.select<(SZ >> 3), 1, 8, 1>(0, 8) =
      src2.template bit_cast<float, (SZ >> 3), 8>();
...
ESIMD_NODEBUG ESIMD_INLINE
  ...
  src_0.template select<N, 1>() = src0.template bit_cast<ushort>();
