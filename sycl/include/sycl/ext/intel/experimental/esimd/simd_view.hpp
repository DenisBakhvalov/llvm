...
protected:
  simd_view_impl(BaseTy &Base, RegionTy Region) : M_base(Base), M_region(Region) {}
  simd_view_impl(BaseTy &&Base, RegionTy Region) : M_base(Base), M_region(Region) {}
...
public:
  ...
  simd_view_impl &operator=(const simd_view_impl &Other) { return write(Other.read()); }
  ...
  /// View as a 2-dimensional simd_view_impl.
...
private:
  simd_view(BaseTy &Base, RegionTy Region) : simd_view_impl<BaseTy, RegionTy>(Base, Region) {}
  simd_view(BaseTy &&Base, RegionTy Region) : simd_view_impl<BaseTy, RegionTy>(Base, Region) {}
...
public:
  ...
  using simd_view_impl<BaseTy, RegionTy>::operator=;
};*/
...
class simd_view<BaseTy, region_base<Is2D, T, 1, StrideY, 1, StrideX>> : public simd_view_impl<BaseTy, region_base<Is2D, T, 1, StrideY, 1, StrideX>> {
  public:
...
template <typename BaseTy, bool Is2D, typename T, int StrideY, int StrideX>
class simd_view<BaseTy, region_base_1<Is2D, T, StrideY, StrideX>> : public simd_view_impl<BaseTy, region_base_1<Is2D, T, StrideY, StrideX>> {
  template <typename, int> friend class simd;
