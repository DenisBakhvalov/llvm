...
void test_simd_view_conv() SYCL_ESIMD_FUNCTION {
  simd<int, 4> v0 = 1;
  int x = v0.select<1, 0>(0);
  //int x2 = v0.select<2, 0>(0); // error
}

void test_simd_view_amb() SYCL_ESIMD_FUNCTION {
  simd<int, 4> v1 = 1;
  simd<int, 4> v2 = 2;

  //bool res = v1[0] == 0;
  // 1. simd_view -> ushort -> built-in
  // 2. simd_view::== ()
...
// simd<T, 1> + T are ambiguous. This test checks that there is a specific
// overload for simd<T, 1> + T operations.
