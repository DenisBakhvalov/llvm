...
its shape to 1D or 2D object through the ```bit_cast``` function:
- ```bit_cast<EltTy>( )```: returns a reference to the calling simd object interpreted as a new
simd vector with the size determined by the template **EltTy** parameter.
- ```bit_cast<EltTy, Height, Width>( )```: returns a reference to the calling simd object interpreted
...
such as row/column operators and 2D select/replicate/bit_cast/merge operations.

```cpp
  simd<float, 32> v1;
  auto m1 = v1.bit_cast<float, 4, 8>();
  simd<float, 4> v2;
  auto m2 = v2.bit_cast<float, 2, 2>();
